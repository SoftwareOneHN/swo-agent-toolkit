...
function benchmarkSkill(category: string, skillName: string): SkillBenchmark {
  ...
  return {
    category,
    skillName,
    skillPath: `registry/skills/${category}/${skillName}/SKILL.md`,
    tokensWithSkill,
...
async function main() {
  ...
  const relativeArchivePath = `tools/benchmarks/archive/v${version}.md`;
  ...
  updateReadmeHistory(path.join(ROOT_DIR, 'packages/cli/README.md'), history.records);
