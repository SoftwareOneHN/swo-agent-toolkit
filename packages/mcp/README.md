...
The server honours the same tier model as `swo-agent-toolkit`'s index generator: broad globs (`**/*.dart`) only match if the skill is the registered `base_language_skills` for that category. Everything else is demoted to keyword match.
...
```bash
npx @swohn/cli@latest init    # asks about MCP during setup
npx @swohn/cli@latest sync    # installs MCP at the chosen scope
```
...
on the MCP server "swo-agent-toolkit". Use the returned SKILL.md content as
