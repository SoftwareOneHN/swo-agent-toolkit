...
This document explains how `@swohn/mcp` works internally — for contributors, security reviewers, and integrators evaluating whether to ship it inside their stack.
...
- A skill installer — that's the CLI's job (`swo-agent-toolkit sync`).
...
| AGENTS.md found but no `skills/` directory | Server starts; `setup.kind === 'no-skills-dir'` | Tools return: "run `swo-agent-toolkit sync` to install skills" |
