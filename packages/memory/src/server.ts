 ...
 *   memory_snapshot_file, memory_log_session, memory_ingest, memory_clusters
 */
...
export function createBrainServer(
      ...
      if (!entry) {
        return {
          content: [
            {
              type: 'text' as const,
              text: `No memory found with hash prefix "${params.hash_prefix}". Try memory_query to find available memories.`,
            },
          ],
        };
      }
  ...
  server.tool(
    ...
    'Store a chat session history linked to a brain memory. Enables recalling full conversations.',
    {
      ...
      project: z.string().optional().describe('Project name'),
      memory_hash: z.string().optional().describe('Hash of related memory_store memory'),
