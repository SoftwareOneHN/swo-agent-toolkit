      ...
      if (parsed) {
        ...
        if (treeResult && Array.isArray(treeResult.tree)) {
          ...
          allFiles.forEach((f: GitHubTreeItem) => {
            if (f.path.startsWith('registry/skills/') && f.type === 'tree') {
              const parts = f.path.split('/');
              if (parts.length === 3) foundCategories.add(parts[2]);
            }
          });
          ...
          const workflows = allFiles
            .filter(
              (f) =>
                f.path.startsWith('registry/workflows/') &&
                f.path.endsWith('.md'),
            )
        ...
        new Set(
          treeResult.tree
            .filter((f: GitHubTreeItem) =>
              f.path.startsWith(`registry/skills/${framework}/`),
            )
            .map((f: GitHubTreeItem) => f.path.split('/')[3])
            .filter(Boolean),
        ),
      );
    } catch (error) {
