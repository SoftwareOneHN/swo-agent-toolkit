 ...
 * Guaranteed to execute on any machine running swo-agent-toolkit.
 ...
 * Installed by swo-agent-toolkit (ags sync). Remove via: ags hooks uninstall
...
process.stdin.on('end', () => {
  try {
    ...
    console.log(
      ...
      'swo-agent-toolkit MCP. It returns applicable SKILL.md rules, ' +
...
description: Load matching skill rules from swo-agent-toolkit before editing source files
triggers:
  - type: file_change
    fileFilter: "**/*"
instructions: |
  Before editing any source file, call load_skills_for_files(files=[<absolute_file_path>])
  on the swo-agent-toolkit MCP server. It returns applicable SKILL.md rules for the
...
const CLAUDE_MCP_PERMISSION = 'mcp__swo-agent-toolkit__*';
