...
export class SkillSyncService {
  ...
  private async fetchSkill(
    ...
    const prefix = `registry/skills/${sourceCat}/${skillName}/`;
  ...
  private identifyFoldersToSync(
    ...
    const skillFolders = new Set(
      tree
        .filter((f) => f.path.startsWith(`registry/skills/${category}/`))
  ...
  private expandAbsoluteInclude(
    ...
    if (targetSkill === '*') {
      const catSkills = new Set(
        tree
          .filter((f) => f.path.startsWith(`registry/skills/${targetCat}/`))
          .map((f) => f.path.split('/')[3])
          .filter(Boolean),
      );
      ...
      if (
        tree.some((f) =>
          f.path.startsWith(`registry/skills/${targetCat}/${targetSkill}/`),
        )
      ) {
