...
describe('SkillSyncService', () => {
  ...
  describe('assembleSkills', () => {
    ...
    it('should assemble skills correctly including absolute and relative', async () => {
      ...
      mockGithubService.getRepoTree.mockResolvedValue({
        tree: [
          { path: 'registry/skills/cat1/s1/SKILL.md', type: 'blob' },
          { path: 'registry/skills/other/s2/SKILL.md', type: 'blob' },
        ],
      });
  ...
  describe('identifyFoldersToSync & expandAbsoluteInclude', () => {
    it('should handle wildcard * and skip duplicates', () => {
      const tree = [
        { path: 'registry/skills/other/s1/SKILL.md', type: 'blob' },
      ] as any[];
    ...
    it('should exclude folder if not in include list', () => {
      ...
      const tree = [{ path: 'registry/skills/test/s1/', type: 'tree' }] as any[];
      // @ts-ignore - private
  ...
  describe('fetchSkill Filtering', () => {
    it('should filter files correctly', async () => {
      const tree = [
        { path: 'registry/skills/c/s/SKILL.md', type: 'blob' },
        { path: 'registry/skills/c/s/references/f', type: 'blob' },
        { path: 'registry/skills/c/s/scripts/f', type: 'blob' },
        { path: 'registry/skills/c/s/assets/f', type: 'blob' },
        { path: 'registry/skills/c/s/ignored', type: 'blob' },
      ];
      mockGithubService.downloadFilesConcurrent.mockImplementation(
        (t: { path: string }[]) =>
    ...
    it('should handle relative vs absolute skill fetch', async () => {
      const tree = [{ path: 'registry/skills/other/s/SKILL.md', type: 'blob' }];
      mockGithubService.downloadFilesConcurrent.mockResolvedValue([
        { path: 'registry/skills/other/s/SKILL.md', content: 'c' },
      ]);
      // @ts-ignore - private
      const res = await skillSyncService.fetchSkill(
        'o',
