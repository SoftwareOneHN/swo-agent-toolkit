...
describe('RegistryService', () => {
  ...
  describe('discoverRegistry', () => {
    ...
    it('should discover categories and metadata successfully (lines 25-36 coverage)', async () => {
      ...
      mockGithub.getRepoTree.mockResolvedValue({
        tree: [{ path: 'registry/skills/react', type: 'tree' }],
      });
    ...
    it('should discover workflows from registry/workflows/ path (line 66 coverage)', async () => {
      mockGithub.getRepoTree.mockResolvedValue({
        tree: [
          { path: 'registry/workflows/test-wf.md', type: 'blob' },
          { path: 'registry/workflows/readme.txt', type: 'blob' }, // should be filtered out
        ],
      });
      const result = await service.discoverRegistry('url');
    ...
    it('should ignore deep paths or non-category paths (line 41 coverage)', async () => {
      mockGithub.getRepoTree.mockResolvedValue({
        tree: [
          { path: 'registry/skills/flutter/extra', type: 'tree' }, // 4 parts (too deep), should be ignored
  ...
  describe('getFrameworkSkills', () => {
    it('should list framework skills', async () => {
      mockGithub.getRepoTree.mockResolvedValue({
        tree: [{ path: 'registry/skills/flutter/s1/', type: 'tree' }],
      });
