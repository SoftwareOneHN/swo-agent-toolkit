...
/**
 * Command for upgrading the `swo-agent-toolkit` CLI to the latest version.
...
export class UpgradeCommand {
  ...
  async run(options: { dryRun?: boolean }) {
    ...
    try {
      latestVersion = execSync('npm view swo-agent-toolkit version', {
        encoding: 'utf8',
      }).trim();
    } catch {
  ...
  private getUpgradeCommand(
    ...
    switch (pm) {
      case 'pnpm':
        return `pnpm add -g swo-agent-toolkit@${version}`;
      case 'yarn':
        return `yarn global add swo-agent-toolkit@${version}`;
      default:
        // Default to npm install -g
        return `npm install -g swo-agent-toolkit@${version}`;
    }
  }
  ...
  private async detectAndFixStaleShim(latestVersion: string): Promise<void> {
    ...
    try {
      ...
      if (shimContent.includes(`swo-agent-toolkit@${latestVersion}`)) {
        this.printShimPathHint();
        return;
      }
      ...
      if (!existsSync(correctShimPath)) {
        console.log(
          pc.yellow(
            `⚠️  Correct shim not found at ${correctShimPath}. Re-run: ${pc.bold('pnpm add -g swo-agent-toolkit@latest')}`,
          ),
        );
        this.printShimPathHint();
        return;
  ...
  private printManualInstructions(
    ...
    if (pm !== 'npm') {
      ...
      console.log(
        pc.gray(
          `  ${sudoPrefix}npm install -g swo-agent-toolkit@${version}`,
        ),
      );
    }

    console.log(pc.gray('\nOr run via npx (no install required):'));
    console.log(pc.gray(`  npx swo-agent-toolkit@${version} sync`));
  }
}
